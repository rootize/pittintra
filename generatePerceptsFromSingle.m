function percepts = generatePerceptsFromSingle(percepts,startid,videofile,description )
%GENERATEPERCEPTSFROMSINGLE Summary of this function goes here
%   Detailed explanation goes here


fps=local_getFPS(videofile); 
[file_dir,file_name,~]=fileparts(videofile);
%  results=load([file_name,'.mat']);
 results=local_generateResultsFromSingle(fullfile(file_dir,[file_name,'_mat']));
...
function results=local_generateResultsFromSingle(filepath)

idx=1;
% landmarks_info=[];
list_lm_files=dir(fullfile(filepath,'*_lm_info.mat'));
num_lm_files=length(list_lm_files);
for i=1:1:num_lm_files
 load(fullfile(    filepath,list_lm_files(i).name));
  landmarks_info(i)=temp_lm;

end
idx=1;
% rects_info=struct([]);
list_rect_files=dir(fullfile(filepath,'*_rect_info.mat'));
num_rect_files=length(list_rect_files);
for i=1:1:num_rect_files
 load(fullfile(filepath,list_rect_files(i).name));
 rects_info(i)=temp_mat;
   
end
results.rects_info=rects_info;
results.landmarks_info=landmarks_info;
end
